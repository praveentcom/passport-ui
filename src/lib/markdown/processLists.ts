...
function processLists(html: string): string {
  ...
  const flushList = () => {
    if (currentListType && listItems.length > 0) {
      const listClass =
        currentListType === "ol"
          ? "list-decimal pl-0 ml-4 mb-5 [&>li]:pl-1"
          : "list-disc pl-0 ml-4 mb-5 [&>li]:pl-1";
  ...
  for (const line of lines) {
    ...
    if (taskUncheckedMatch) {
      ...
      listItems.push(
        `<li class="ml-1 mb-0.75 text-sm leading-relaxed">${unorderedMatch[1]}</li>`
      );
      ...
      listItems.push(
        `<li class="ml-1 mb-0.75 text-sm leading-relaxed">${orderedMatch[1]}</li>`
      );
    } else {
      flushList();
      if (line.trim()) {
