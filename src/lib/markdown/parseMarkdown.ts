...
function parseMarkdown(content: string): string {
  ...
  let processedContent = content
    .replace(/^###### (.*$)/gim, '<h6 class="mb-4.5 mt-5.5">$1</h6>')
    .replace(/^##### (.*$)/gim, '<h5 class="mb-4.5 mt-5.5">$1</h5>')
    .replace(/^#### (.*$)/gim, '<h4 class="mb-4.5 mt-5.5">$1</h4>')
    .replace(/^### (.*$)/gim, '<h3 class="mb-4.5 mt-5.5">$1</h3>')
    .replace(/^## (.*$)/gim, '<h2 class="mb-4.5 mt-5.5">$1</h2>')
    .replace(/^# (.*$)/gim, '<h1 class="mb-4.5 mt-5.5">$1</h1>');
  ...
  processedContent = processedContent
    .replace(/\n\n/g, '</p><p class="mb-4.5">')
    .replace(/^(?!<[h|l|p|c|b|i|d|u|t])/gm, '<p class="mb-4.5">')
    .replace(/(?<!>)$/gm, "</p>")
    .replace(/<p class="mb-4.5"><\/p>/g, "")
    .replace(/<p class="mb-4.5">(<[h|l|p|c|b|i|d|u|t])/g, "$1");
  ...
  if (footnoteRefs.length > 0) {
    let footnotesHtml =
      '<div class="footnotes mt-8 pt-4 border-t border-border/50"><h4 class="text-sm font-medium mb-4.5">Footnotes</h4><ol class="text-xs space-y-2">';
